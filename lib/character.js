...
class Character {
  constructor(name, atk = 1, def = 1, hp = 10) {
    this.name = name;
    this.stats = new Stats(atk, def, hp)
  ...
  equip(item, slot) {
    if (this.inventory.equip(item, slot)) { // will return true or false
  ...
  unequip(type) {
    let unequipItem = this.inventory[type];
    ...
    this.inventory[type] = new Item(type);
    console.log('');
    console.log(`${this.name} unequipped ${unequipItem.name}!`);
  }
...
let monster = new Character('Slime', 5, 0, 30);
