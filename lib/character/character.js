...
const abilityUtil = require('../skills/utils');
...
class Character {
  ...
  learnCombat(skill, resourceParam) {
    let newSkill = combat[skill];
    this.combatAbilities[skill] = newSkill(resourceParam).bind(this);
    console.log('');
    console.log(`${this.name} learned ${skill}!`);
  }

  learnSpell(spell, resourceParam) {
    let newSpell = spellbook[spell];
    this.spellBook[spell] = newSpell(resourceParam).bind(this);
    console.log('');
    console.log(`${this.name} learned ${spell}!`);
  }
