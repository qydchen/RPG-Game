...
class PlayerCharacter extends Character {
  ...
  cast(skill, ...targets) {
    this.useSkill(skill, this.spellBook, 'spellbook', ...targets);
  }

  perform(skill, ...targets) {
    this.useSkill(skill, this.combatAbilities, 'combat ability', ...targets);
  }

  useSkill(skill, abilities, txt, ...targets) {
    try {
      if (abilities[skill]) {
        let ability = abilities[skill];
        return ability.use(...targets);
      } else {
